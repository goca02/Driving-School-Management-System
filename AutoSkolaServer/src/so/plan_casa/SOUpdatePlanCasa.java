 ...
 * @author Goca
 */
public class SOUpdatePlanCasa extends AbstractSO {
    ...
    protected void validacija(AbstractDomainObject ado) throws Exception {
        ...
        ArrayList<PlanCasa> planovi = (ArrayList<PlanCasa>) (ArrayList<?>) DBBroker.getInstance().select(ado);

        for (PlanCasa plan : planovi) {
            if (plan.getPlanCasaID() != pc.getPlanCasaID()) {
